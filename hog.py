...
Eeshan Agarwal: eeshanagarwal@berkeley.edu: cs61a-akj
AND
Pratham Makhni Alag: prathamalag@berkeley.edu: cs61a-akk
Sumbitted on: Wednesday, 09/18/2013 """
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    average = 0
    no_of_dice = 1
    while test_no_of_dice <= 10:
        new_average = make_averaged(roll_dice, 1000)(test_no_of_dice, dice)
        print (test_no_of_dice,"dice scores",new_average,"on average")
        if (new_average > average):
            average = new_average
            no_of_dice = test_no_of_dice
        test_no_of_dice += 1
    return no_of_dice
...
def bacon_strategy(score, opponent_score):
    ...
    """
    if opponent_score // 10 == 0:
        new_opp_score = opponent_score + 1
    else:
        new_opp_score = max(opponent_score % 10, opponent_score // 10) + 1
    if new_opp_score >= BACON_MARGIN:
        return 0
    else:
...
def swap_strategy(score, opponent_score):
    ...
    if opponent_score // 10 == 0:
        ...
        new_opp_score = max(opponent_score % 10, opponent_score // 10) + 1
    if (score + new_opp_score) == 2 * opponent_score:
        return BASELINE_NUM_ROLLS
    elif 2 * (score + new_opp_score) == opponent_score:
        return 0
    else:
...
def final_strategy(score, opponent_score):
    ...
    In order to come up with a final strategy, we came up with lots of logics (about 15) including those for
    what to do when one has a specifc high or low score, etc and put them in this function as elif 
    statements with the else being 'return bacon_strategy(score, opponent_score)'. This however did not create
    a high enough average win rate and we therfore tried deleting a few, so that bacon_strategy being the else
    statement could get precedence and be called soon. The following code seems to work very well as it uses only
    the rules that can be most beneficial and is in perfect order so that bacon_strategy may also 
    ...
    if opponent_score // 10 == 0:
        ...
        new_opp_score = max(opponent_score % 10, opponent_score // 10) + 1
    sum_of_scores = score + new_opp_score
    if sum_of_scores == 2 * opponent_score:
        return BASELINE_NUM_ROLLS
    elif 2 * sum_of_scores == opponent_score:
        return 0
    elif 2 * (score + 1) == opponent_score:
        return 10
    elif (sum_of_scores + opponent_score) % 7 == 0:
        return 0
    else: 
        return bacon_strategy(score, opponent_score)
